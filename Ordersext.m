...
/////////////////////////////////////////////////////
// Weak equivalence classes in orders in etale algebras over \Q
...
version 0.9->0.10: new stuff has been moved in the various packages. cleaning of the code. publication on the webpage 13/04
version 0.8->0.9: added MultiplicatorRing(Order)
version 0.7->0.8: added intrinsic '/'(a::AlgAssElt,b::AlgAssElt)->AlgAssElt
version 0.6->0.7: ???
version 0.5->0.6: corrected a bug in IsIsomorphic2
version 0.4->0.5: in IsWeakEquivalent and IsIsomorphic2 I redifine the ideals over the MultiplicatorRing.
version 0.3->0.4: '*' is over-written, the old version of the code is commented;
                  IsFiniteEtale is included as a requirement;
                  some functions have been moved to AbelianVarieties_0.1.m ;
                  some functions have been moved to usefulfunctions.m ;
version 0.2->0.3: IsProductOfOrders and IsProductOfIdeals have been changed;
version 0.1->0.2: HomsToC was modified;
*/
/* List of functions
...
-TOBETESTED: a function PrimesAbove(I), that lists all the prime ideals P|I, for I also non invertible
-DROPPED: changed to a test IsFiniteEtale Create a class AlgSep or AlgEt (for étale algebra) and work with them... so that I don't have to over-write with functions that may not work in the non-commutative case.
*/
...
{given an integral S-ideal, returns if the ideal is a prime fractional ideal of S, that is a maximal S ideal}
     require IsIntegral(I): "the ideal must be integral";
     prim:=PrimesAbove(I);
     if #prim eq 1 and I eq prim[1] then return true; else return false; end if;
end intrinsic;
...
{returns if A is a finite etale algebra over Q, that is a finite product of number fields}
  ...
  else //it does not recognize the BaseRing!!!
...
{returns the smallest integer contained in the ideal I}
     ...
     for i in [1..#divs-1] do
        if divs[i] in I and not divs[i+1] in I then return divs[i]; end if;
     end for;
...
assert d*I subset S;
    d:=Integers() ! d;
    if d eq 1 then return true,d;
    else
      for y in Divisors(d) do if (y*I) subset S then
...
{returns all the overorders of E}
  if assigned E`OverOrders then return E`OverOrders;
  else
    A:=Algebra(E);
    require IsFiniteEtale(A): "the algebra of definition must be finite and etale over Q";
    if assigned A`MaximalOrder then O:=A`MaximalOrder; else O:=MaximalOrder(A); A`MaximalOrder:=O; end if;
    if O eq E then E`OverOrders:=[E]; return [E]; end if;
...
//15/02/2018 we add the LowIndexProcess
    ...
    if assigned E`OverOrders then return [S: S in E`OverOrders | S subset O];
    else
...
{checks if all the over orders of S are Gorenstein}
  require IsFiniteEtale(Algebra(S)): "the algebra of definition must be finite and etale over Q";
  if assigned S`OverOrders then seqOO:=S`OverOrders;
    else seqOO:=FindOverOrders(S); S`OverOrders:=seqOO;
  end if;
...
{computes the conductor of an order}
  A:=Algebra(O);
  require IsFiniteEtale(A): "the algebra of definition must be finite and etale over Q";
  if assigned A`MaximalOrder then OA:=A`MaximalOrder;
  else OA:=MaximalOrder(A); A`MaximalOrder:=OA; end if;
return ColonIdeal(O,ideal<O|ZBasis(OA)>);
end intrinsic;

intrinsic IsWeakEquivalent(I::AlgAssVOrdIdl,J::AlgAssVOrdIdl)->BoolElt
{ checks if 1 \in (I:J)*(J:I) }
  require IsFiniteEtale(Algebra(I)): "the algebra of definition must be finite and etale over Q";
  S := MultiplicatorRing(I);
  if MultiplicatorRing(J) ne S then return false;
  else
    IS:=ideal<S|ZBasis(I)>;
    JS:=ideal<S|ZBasis(J)>;
...
{given given a sequence of number fields, it returns the associative algebra given by the product of the number field}
  ...
  for i in [1..#num_fields] do
	deg_fi:=Degree(num_fields[i]);
        if i eq 1 then deg_prev:=0; else deg_prev:=&+[Degree(num_fields[j]) : j in [1..(i-1)]]; end if;
        if i eq #num_fields then deg_next:=0; else deg_next:=&+[Degree(num_fields[j]) : j in [(i+1)..(#num_fields)]]; end if;
        Ai:=num_fields[i];
...
/*
//old code see above for the new version
intrinsic SplittingAlgebra(A::AlgAss) -> AlgAss, Map
{ given a product of number fields A, returns the product of the splitting fields and a map from A }
    require IsFiniteEtale(A): "the algebra of definition must be finite and etale over Q";
...
{ compute the nth power of an ideal }
  ...
  power_positive:=function(I,n)
      ...
      for i in [1..#bin_exp] do
        if bin_exp[i] eq 1 then Append(~squares_id,id); end if;
           id:=id*id;
      end for;
      output:=squares_id[1];
  ...
  if n eq 0 then
      ...
      if n gt 0 then return power_positive(I,n); end if;
      if n lt 0 then

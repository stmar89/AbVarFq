...
{return the PicardGroup of the order S, which is not required to be maximal, and a map from the PicardGroup to a set of representatives of the ideal classes.
    ...
    ReorderGenerators:=function(P,pmap)
            ...
            if k eq 1 then
                for i in [1..#O_asProd] do
                    ...
                    if F_indexes[i] mod p eq 0 then
                        ...
                        primes_above_p[<i,p>] := Prune(SplitPrime(L, p)); //SplitPrime is defined in the sorting module
                    end if;
                end for;
            end if;
            for i in [1..#O_asProd] do
        ...
        Pquo := P;
        Pgens := Generators(P);
        new_prime_lifts := prime_lifts;
        new_prime_list := prime_list;
        for gen_num in [#Pgens..1 by -1] do
            ...
            for i in [1..#prime_list] do
                new_lift := quo_proj(old_prime_lifts[i]);
                if new_lift in Psub then
                    continue;
                end if;
                Append(~new_prime_list, prime_list[i]);
                Append(~new_prime_index, prime_indexes[i]);
                Append(~new_prime_lifts, new_lift);
                ord := Order(new_lift);
                if not IsDefined(best_of_order, ord) then
                    best_of_order[ord] := #new_prime_list;
                end if;
                Psub := sub<Pquo|[g : g in Generators(Psub)] cat [new_lift]>;
            end for;
        end for;
...
assert #newgens eq #Generators(P);
assert forall{ I : I in newgens | Order(I) eq S};
assert forall{ I : I in newgens | MultiplicatorRing(I) eq S};
AbelianInvariants(P);
[Order(P.i) : i in [1..Ngens(P)]];
[gen@@pmap : gen in newgens];
[Order(gen@@pmap) : gen in newgens];

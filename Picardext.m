...
{return the PicardGroup of the order S, which is not required to be maximal, and a map from the PicardGroup to a set of representatives of the ideal classes.
    ...
    At each step, we find a the smallest prime ideal of the correct order not in the span
    ...
    */
    ReorderGenerators:=function(P,pmap)
        ...
        expand_factor_base := procedure(~q, ord_goal, curquo, ~primes_above_p, ~primes_by_norm, ~orders_of_primes, ~prime_lifts, ~gens, ~Pgens)
        ...
        trim_factor_base := procedure(curquo, ~primes_by_norm, ~orders_of_primes, ~prime_lifts)

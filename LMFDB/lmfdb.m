...
/**************************************************************************
    ...
    LMFDB LABELS OF IDEALS IN NUMBER FIELDS

    I took this from
    https://github.com/JohnCremona/sorting/blob/master/code/sorting.m 
    on 14/06/17

    I've made only content free changes (providing intrinsics, etc)
...
function IdealFromNormIndex(K,n,m)
    ...
    I:=ideal<OK|1>;
    if n eq 1 then
        error if m ne 1, Sprintf("No ideal with norm %o and index %o exists in %o", n, m, K);
        return I;
    end if;
    pp:=[p[1]^p[2]:p in Factorization(n)];
    Q:=<IdealsOfPrimePowerNorm(K,q):q in pp>;
    ...
    error if a ne 0, Sprintf("No ideal with  norm %o and index %o exists in %o", n, m, K);
    return I;
end function;
...
intrinsic LMFDBLabel(I::RngOrdIdl) -> MonStgElt
{The name of the ideal I in the LMFDB database}

    K := NumberField(Order(I));
    require IsAbsoluteField(K) and IsSimple(K) :
        "The number field must be an extension of Q by a single defining polynomial";

    return IdealLabel(I);
end intrinsic;

intrinsic LMFDBIdeal(K::FldNum, s::MonStgElt) -> RngOrdIdl
{The ideal of Integers(K) that has the name 's' in the LMFDB database}

    require IsAbsoluteField(K) and IsSimple(K) :
        "The number field must be an extension of Q by a single defining polynomial";

    return IdealFromLabel(K, s);
end intrinsic;

intrinsic LMFDBIdeal(O::RngOrd, s::MonStgElt) -> RngOrdIdl
{The ideal of the maximal order O that has the name 's' in the LMFDB database}

    K := NumberField(O);
    require IsAbsoluteField(K) and IsSimple(K) :
        "The number field must be an extension of Q by a single defining polynomial";

    return O !! IdealFromLabel(K, s);
end intrinsic;

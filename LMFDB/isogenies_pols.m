...
// declare attributes AlgEtQOrd : ???
    ...
    // TODO for non ConjugateStableOrders
    ...
    isom_cl:=ICM(R); // TODO use canonical reps
    AllMinimalIsogenies:=AssociativeArray();
    for J in isom_cl do
        // J is over R
        ...
        for L in Ls do
            ...
            for i in [1..#wkS] do
                ...
                test_wk,cLW,cWL:=is_weak_eq_same_mult_ring(S!!L,W) 
                if test_wk then
        ...
        AllMinimalIsogenies[J]:=MinimalIsogeniesToJ;
    end for;

    depth:=2; // we look for polarizations that are composition of depth minimal isogenies from the array AllMinimalIsogenies
